...
public class RPGAction { //Wrapper to hold player and AI actions from their figures to make them perform said task. Not designed for movement or shopping.
	...
	public boolean isValid() {
		if ((action == "attack") && (targets.size() != 0))
			return true;
		else if ( (action == "special") && (targets.size() != 0) && (specialNum >= 0) )
			return true;
		else if ( (action == "inventory") && (targets.size() != 0) && ((inventoryAction == "use") || (inventoryAction == "drop") || (inventoryAction == "give")) && (inventorySlot != 0))
			return true;
		else if ((action == "run"))
			return true;
		else return false;
